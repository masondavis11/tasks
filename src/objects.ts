...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    if (newAnswer === question.expected.trim().toLocaleLowerCase()) {
        return true;
    } else {
        return false;
    }
...
export function toShortForm(question: Question): string {
    ...
    const questionName = question.name.slice(0, 10);
...
export function renameQuestion(question: Question, newName: string): Question {
    const newNameQuestion = { ...question, name: newName };
...
export function publishQuestion(question: Question): Question {
    const newPublished = question.published ? false : true;
    const newPublishedQuestion = { ...question, published: newPublished };
    return newPublishedQuestion;
}
